...
def human(v):
    for d,n in [(1e15,"quadrillion"),(1e12,"trillion"),(1e9,"billion"),(1e6,"million"),(1e3,"thousand")]:
        if v>=d:return f"{v/d:,.3f} {n}"
    return f"{v:,.0f}"

def print_report(args,gpu,model,capacity,estimates):
    ...
    for e in estimates: print(f"{e.scenario:<14}{e.prompt_tps:>15,.1f}{e.output_tps:>15,.1f}{e.per_user_tps:>17,.2f}{e.ttft_s:>9.2f}s{e.itl_ms:>9.1f}ms")
    print("-"*84)
    ...
    for label,seconds in [("hour",3600),("day",86400),("week",604800),("30-day month",2592000),("365-day year",31536000)]:
        v=nominal.output_tps*seconds; print(f"Output tokens/{label:<12}: {v:>20,.0f}  ({human(v)})")
    if not fits: print("\nWARNING: requested workload does not fit in usable VRAM; throughput assumes all-GPU execution.")

def write_json(path,args,gpu,model,capacity,estimates):
    payload={"inputs":vars(args),"gpu":asdict(gpu),"model":asdict(model),"capacity":capacity,"estimates":[asdict(e) for e in estimates]}
